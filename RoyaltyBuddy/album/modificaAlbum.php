<?php
	...
	$traduttore = new NtxTraduttore("ALB_000_005", "IT", $_SESSION[NTX_LINGUA_DEST]);
	//$traduttore = new NtxCreaTraduzione("ALB_000_005", "IT", __FILE__);
	
	$next = "dettagliAlbum.php";
	$back = $_SESSION['page'];
	$idAlbum = $_SESSION['idSelect'];
	$idUtente = $_SESSION['idUtente'];
	
	$sql = " SELECT * FROM album WHERE idAlbum = '$idAlbum' AND validate = 1 ";
	$utente = " SELECT  utenti.nome, utenti.cognome
				FROM 	utenti
	...
	<body>
		<div data-role="page">
			<div data-role="header" data-theme="b" data-position="fixed">
				<h1>
					<?php $traduttore->html('Modifica Album') ?>
				</h1>
			...
			<div role="main" class="ui-content" data-theme="a">
				<!-- Lista Campi -->
				<form id="formModAlbum" name="formModAlbum" method="post">
					<div>
						<div>
							<label for="iAlbum"><?php $traduttore->html('Album') ?></label>
							<input type="text" class="capitalize" name="iAlbum" id="iAlbum" value="<?php echo $html['nomeAlbum'] ?>">
						</div>
						<div>
							<label for="iAnno"><?php $traduttore->html('Anno di pubblicazione') ?></label>
							<input type="text" name="iAnno" id="iAnno" value="<?php echo $html['annoPubblicazione'] ?>">
						</div>
						<div>
						...
						<div>
							<input type="hidden" name="idUtente" id="idUtente" value="<?php echo $idUtente ?>" readonly>
						</div>
					</div>
					<br>
					<br>
					<!-- Btn Salva -->
					<div>
						<input type="submit" name="iSalva" id="iSalva" value="<?php $traduttore->html('Salva') ?>" onclick="modAlbum(formModAlbum)" />
					</div>
				</form>
			</div>
		</div><!-- /page -->
		...
		<div id=divMessage style="display:none" class="modal">
			<div id=divMessage2 style="text-align:center" class="modal-content">
				...
				<div id="messaggioVero">
					<p>
						<?php $traduttore->html('Modificare le informazioni dell\'album') ?>
					</p>
				</div>
			</div>
		</div>
