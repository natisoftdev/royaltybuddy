<?php
	...
	$traduttore = new NtxTraduttore("EVE_000_004", "IT", $_SESSION[NTX_LINGUA_DEST]);
	//$traduttore = new NtxCreaTraduzione("EVE_000_004", "IT", __FILE__);
	
	$idUtente = $_SESSION['idUtente'];
	$home = $_SESSION['home'];
	
	$sql = " SELECT nome, cognome, email FROM utenti WHERE idUtente = '$idUtente' AND validate = 1";
	$eventi = "	SELECT  eventi.idEvento, eventi.nomeEvento, eventi.nomeLuogo, eventi.dataEvento
				FROM  	eventi 
				WHERE 	eventi.idUtente = $idUtente AND eventi.validate = 1
				ORDER BY eventi.dataEvento";
	...
	while($rs=ntxRecord($queryEventi))
	{
		...
		$listaEventi .= "<li><a onclick='openData(\"$html[idEvento]\",\"dettagliEvento.php\",\"listaEventi.php\")'>$html[nomeEvento] - $html[nomeLuogo] - $html[dataEvento]</a></li>";
	}
		...
		<div data-role="page">
			...
			<div role="main" class="ui-content">
				<div class="float_right" style="width:auto">
					<a onclick="openPage('aggiungiEvento.php','listaEventi.php')">
						<?php $traduttore->html('Aggiungi') ?>
					</a>
				</div>
				<div>
					<label class="lista" for="iEventi"><?php $traduttore->html('Lista Eventi') ?></label>
					<ul data-role="listview" data-filter="true" data-filter-placeholder="<?php $traduttore->html('Cerca evento...') ?>" data-inset="true" name="iEventi">
						<?php
							echo $listaEventi;
						?>
					</ul>
		...
		<div id=divMessage style="display:none" class="modal">
			<div id=divMessage2 style="text-align:center" class="modal-content">
				...
				<div id="messaggioVero">
					<p>
						<?php $traduttore->html('Lista degli eventi presenti') ?> <br><?php $traduttore->html('( usare la ricerca per filtrarli )') ?>
					</p>
				</div>
			</div>
		</div>
